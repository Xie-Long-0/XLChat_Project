...
- 群聊与媒体消息尚未 E2EE（M7b/M8），服务端仍可见其明文：M7a 明文群聊服务端业务已上线（子任务一/二），群消息以明文传输与存储，属阶段性形态；但群组接口均遵循先授权再操作（仅成员可发言/邀请/查询，踢人带角色层级保护），输入长度与批量大小受限（群名 ≤64、单批邀请 ≤100、群成员 ≤200、群消息 ≤16384 字符）。
