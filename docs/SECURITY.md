...
- **解密缓存**：已解密消息的明文按 messageId 持久化于本地（同样 DPAPI 保护），一次性预密钥删除后重新同步时由缓存兜底；等价于本地消息存储，不削弱传输侧/服务端不可读的安全属性。
- **离线发送**：对方尚未注册密钥时消息保留在客户端 outbox 并定期重试，不丢弃；本地持久化 outbox 仍未实现（发送方提前退出应用会丢失未送达消息）。
...
- 数据库使用版本化迁移机制（`schema_version` 表，当前 V5），禁止隐式 schema 变更。
...
- 客户端私钥文件在非 Windows 平台为明文存储（仅 Windows 有 DPAPI 保护）。
- 解密缓存含消息明文（DPAPI 加密存于本地），拥有本机用户权限者可读取，与主流 IM 本地存储模型一致。
