...
**当前状态（2026-08-21）：拆三个子任务实施。子任务一（服务端群组数据模型 + 协议定义）与子任务二（业务处理器与 fan-out）已完成并通过自动化测试；子任务三（客户端 UI）待实施。**
...
- 增加群组、成员、角色、邀请、退群、踢人。（子任务一落地协议/数据模型，子任务二落地五个服务端处理器：建群/邀请/退群自动转让群主/踢人层级保护/群信息查询，均先授权再操作）
...
- 群成员变更产生系统消息。（子任务二已落地：contentType=system 结构化消息 + GroupChangedNotification/group_changed 事件）
...
16. M7a 明文群聊（子任务一协议/数据模型、子任务二业务处理器与 fan-out 已完成 → 子任务三客户端 UI） → M7b Sender Keys 群 E2EE。
