...
- [ ] ~~增加本地缓存，避免每次启动都全量拉取。~~（2026-08-03 审查更正：客户端无本地缓存/离线 outbox；2026-08-21 调整：移至 M6.5 本地持久化缓存与持久化 outbox）
- [x] 设计消息 ID：服务端递增 ID。
...
- [ ] ~~客户端重启后仍能显示历史消息。~~（2026-08-03 审查更正：当前依赖重启后重新拉取，非本地持久化；2026-08-21 调整：随 M6.5 本地持久化缓存一并补齐）
...
**当前状态（2026-08-03）：已完成。依据审查结果设立；全部 P0 与大部分 P1 修复已落地，构建通过且 4 组自动化测试全部通过；遗留项：其余命令逐包验 token；本地持久化 outbox 已移至 M6.5。**
...
- [x] 消息幂等：发送请求新增必填 `clientMessageId`，数据库部分唯一索引 `(sender_id, sender_device_id, client_message_id)`，重试返回同一消息；客户端增加内存 outbox 与登录成功后自动重发（本地持久化 outbox 移至 M6.5）。
...
**当前状态（2026-08-21）：未开始。依据路线图核查新增——客户端目前无本地持久化（历史消息每次启动全量拉取、outbox 仅内存），且 E2EE 下本地存储必须加密，越晚补齐改造成本越高，故在 M7 之前落地。**
...
- 新增客户端本地数据库（SQLite、按账号隔离）：会话、消息、联系人、回执缓存；启动后基于 `sync_events` 游标增量同步。
- 内存 outbox 升级为持久化 outbox：未发送消息（含 M6 的对方未注册密钥重试场景）跨重启保留并自动重发。
- 加密存储：消息明文以设备密钥加密后再落库，密钥管理复用 KeyStorage（DPAPI 保护），磁盘上不存在可读明文。
- M6 的解密缓存（按账号+设备持久化）与本地缓存统一归口。
- 登出/切换账号时清除本地数据。
- （可选并行提前项）桌面通知（原 M10）；简化图片消息（原 M8，简化版仅支持小图内联发送，不含断点续传/对象存储）。

### 验收标准（均需自动化测试）

- 客户端重启后仍能显示历史消息（关闭 M3 遗留验收项）。
- 断线/重启后 outbox 消息自动补发且不产生重复（`clientMessageId` 幂等）。
- 本地数据库文件中不存在可读的消息明文。
- 登出清除本地缓存，重新登录可经增量同步恢复。
...
15. M6.5 本地持久化缓存与持久化 outbox（加密存储；可并行提前桌面通知与简化图片消息）。
...
- [x] `clientMessageId` 幂等键 + 部分唯一索引 + 客户端内存 outbox（本地持久化移至 M6.5）。
