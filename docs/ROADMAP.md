...
**当前状态（2026-08-21）：拆三个子任务实施。子任务一（服务端群组数据模型 + 协议定义）已完成并通过自动化测试；子任务二（业务处理器与 fan-out）、子任务三（客户端 UI）待实施。**
...
- 增加群组、成员、角色、邀请、退群、踢人。（子任务一已落地：协议消息类型 60-70、错误码 3009-3012、数据库 V7 群名/角色列与 `DatabaseManager` 群组数据访问方法；业务编排与权限校验在子任务二）
...
16. M7a 明文群聊（子任务一协议/数据模型已完成 → 子任务二业务处理器与 fan-out → 子任务三客户端 UI） → M7b Sender Keys 群 E2EE。
